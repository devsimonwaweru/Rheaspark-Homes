...
import { checkIfUnlocked, getUnlockPrice } from '../lib/paymentUtils';
...
export default function PropertyDetailsModal({ isOpen, onClose, property }) {
  ...
  useEffect(() => {
    const verifyUnlock = async () => {
      ...
      if (user) {
        const status = await checkIfUnlocked(user.id, property.id);
  ...
  const imageSrc = property.image || property.image_url || 'https://via.placeholder.com/400';

  const handleUnlockClick = () => {
    // Redirect to payment with necessary info
    navigate(`/payment?type=unlock&propertyId=${property.id}&price=${unlockPrice}`);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          {/* Contact Section - Locked/Unlocked Logic */}
          <div className={`mt-4 p-4 rounded-xl border ${isUnlocked ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                ...
                <p className="text-gray-800"><span className="font-semibold">Phone:</span> 
