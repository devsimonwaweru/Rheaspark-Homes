...
// --- Sanity Configuration (Copied from AddPropertyModal) ---
...
export default function EditPropertyModal({ isOpen, onClose, property, onSave }) {
  ...
  useEffect(() => {
    if (property) {
      ...
      // Parse Amenities
      try {
        const parsed = typeof property.amenities === 'string' 
      ...
      try {
        ...
        // If images array is empty but image_url exists, use that
  ...
  const handleNewImageChange = (e) => {
    ...
    if (availableSlots <= 0) return alert("Maximum 5 images allowed.");
  ...
  const uploadToSanity = async (file) => {
    ...
    return result.url; // Return the URL string
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Combine existing images with new URLs
      ...
      const updateData = {
        ...
        images: JSON.stringify(finalImages), // Save array as string
        image_url: finalImages[0] || null, // Update main thumbnail
      };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col relative">
          ...
          <div className="flex space-x-1 overflow-x-auto border-b -mb-4">
            {tabs.map(t => (
              <button
                ...
                className={`px-4 py-2 text-sm font-medium capitalize whitespace-nowrap ${activeTab === t ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              >
        ...
        <form onSubmit={handleSubmit} className="p-6 flex-1 overflow-y-auto space-y-4">
          ...
          {activeTab === 'location' && (
            <>
              <Input label="Location / Area" name="location" value={formData.location} onChange={handleChange} />
              <div className="bg-blue-50 p-4 rounded border border-blue-100">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-semibold text-blue-800 text-sm">GPS Coordinates</span>
                  <button type="button" onClick={handleGetLocation} className="text-xs bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700">
          ...
          {activeTab === 'media' && (
            <div className="space-y-4">
              <p className="text-xs text-gray-500 mb-2">Total images allowed: 5. Existing: {existingImages.length}. New: {newFiles.length}.</p>
              ...
              <div className="grid grid-cols-3 gap-3">
                {existingImages.map((img, idx) => (
                  <div key={idx} className="relative group aspect-square">
                    ...
                    <button type="button" onClick={() => removeExistingImage(idx)} className="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity text-xs">
                      &times;
                    </button>
                  </div>
                ))}
                ...
                {previews.map((src, idx) => (
                  <div key={`new-${idx}`} className="relative group aspect-square">
                    ...
                    <button type="button" onClick={() => removeNewFile(idx)} className="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 text-xs">
                      &times;
                    </button>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {amenityOptions.map(opt => (
                    <button type="button" key={opt} onClick={() => toggleAmenity(opt)}
                      className={`px-3 py-1 text-xs rounded border transition-all ${amenities.includes(opt) ? 'bg-blue-100 border-blue-600 text-blue-700' : 'bg-white text-gray-600'}`}>
                      {opt}
                    </button>
                  ))}
                </div>
